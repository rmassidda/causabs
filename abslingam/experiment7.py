...
def plot(
    ...
    results_fname = os.path.join(results_dir, f"experiment7_{flavor}.csv")
    results = pd.read_csv(results_fname)

    print(f"== Experiment 7 ({flavor}) ==")
    ...
    results["params/bootstrap_samples"] = results[
    ...
    for y, y_label in labels:
        ...
        # rename legend
        if hue is not None:
            plt.legend(title=r"Bootstraping", loc="lower right")

        # store plots/exp7_rocauc.pdf
        metric = y.split("/")[1]
        if store:
            plt.savefig(os.path.join(plot_dir, f"exp7_{metric}_{flavor}.pdf"))
            plt.savefig(os.path.join(plot_dir, f"exp7_{metric}_{flavor}.pgf"))
        if show:
            plt.show()
        plt.clf()
...
def run(
    ...
    for run in range(num_runs):
        for n_concrete in [
            ...
            for abs_threshold in [0.1]:
                for bootstrap_samples in [1, 5, 10, 20]:
