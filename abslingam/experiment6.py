...
def plot(
    ...
    results_fname = os.path.join(results_dir, f"experiment6_{flavor}.csv")
    results = pd.read_csv(results_fname)

    print(f"== Experiment 6 ({flavor}) ==")
    ...
    results["params/abs_threshold"] = results["params/abs_threshold"].apply(
        lambda x: f"{x:.0e}"
    )
    ...
    for y, y_label in labels:
        ...
        # rename legend
        if hue is not None:
            plt.legend(title=r"$\mathcal{H}$ Threshold", loc="lower right")

        # store plots/exp6_rocauc.pdf
        metric = y.split("/")[1]
        if store:
            plt.savefig(os.path.join(plot_dir, f"exp6_{metric}_{flavor}.pdf"))
            plt.savefig(os.path.join(plot_dir, f"exp6_{metric}_{flavor}.pgf"))
        if show:
            plt.show()
        plt.clf()
...
def run(
    ...
    for run in range(num_runs):
        for n_concrete in [
            ...
            for abs_threshold in [0.0, 1e-3, 1e-2, 1e-1]:
